...
function create(options) {
	...
	var blank_method = function(){};
	var invalid_method = function(){};
	var commands = [];
	var pingFunctions = [];
	bot.connect();
	bot.onPing(function() {
		for(var i = 0; i < pingFunctions.length; i++) {
			if(pingFunctions[i].running) {
				pingFunctions[i].f();
			}
		}
	});
	...
	*/
	...
	/**
	* Append function to fire on ping (bot pings every 30 seconds)
	* @param {string} key 
	* @param {function} f
	*/
	function onPing(key, f) {
		for(var i = 0; i < pingFunctions.length; i++) {
			if(pingFunctions[i].key == key) {
				console.log(key + ' already exists');
				return;
			}
		}
		pingFunctions[pingFunctions.length] = {'key' : key, 'running' : true, 'f' : f};
	}

	/**
	* Pause function from firing on ping (bot pings every 30 seconds)
	* @param {string} key 
	*/
	function pause(key) {
		for(var i = 0; i < pingFunctions.length; i++) {
			if(pingFunctions[i].key == key) {
				pingFunctions[i].running = false;
			}
		}
	}

	/**
	* Resume function to fire on ping (bot pings every 30 seconds)
	* @param {string} key 
	*/
	function resume(key) {
		for(var i = 0; i < pingFunctions.length; i++) {
			if(pingFunctions[i].key == key) {
				pingFunctions[i].running = true;
			}
		}
	}
	...
	* @param {String} toName
	*/
	function sendMessage(message, roomJid, toName) {
		bot.getRoster(function(err, roster, stanza) {
			...
			if(toName == null || toName == "") {
				bot.message(roomJid, message);
			} else {
				for(var i = 0; i < roster.length; i++) {
					if(roster[i].name == toName) {
